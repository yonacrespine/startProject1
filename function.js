...
function save(){
    ...
    function displayTask() {
        ...
        for(task of arr) {
            ...
            const differenceDay= calculatDayTimeLine(task.timeLine)
            ...
            let html = `<tr>
                        ...
                        <td class="color col-1" style="background-color: ${statusColor}">${task.status}</td>
...
function calculatDayTimeLine(taskTime){
    ...
    const getCurrentDateNewForma = getCurrentDateSplit[2] + "-" + getCurrentDateSplit[1] + "-" + getCurrentDateSplit[0]
...
// function getListFRomDetailsTask(){
//     const detailTask= document.getElementById("detailTask").value 
//     const detail= detailTask.split(`\n`)
   

//     let detailList= `<ul>`

//     detail.forEach((detail)=>{
//         detailList += `<li>${detail}</li>`
//     })
//     detailList+= `</ul>`
...
function validationJS(){
    ...
    if (!titleTask){
        ...
        document.getElementById("messageErrorTitle").innerHTML=" * Missing title *"
    ...
    if (titleTask.length >30  ){
        event.preventDefault()
        document.getElementById("titleTask").focus()
        document.getElementById("messageErrorTitle").innerHTML="* To long title*"
    ...
    if (!detailTask ){
        ...
        document.getElementById("messageErrorTask").innerHTML="* Missing details *"
    ...
    if (detailTask.length <3 || detailTask.length>200 ){
        event.preventDefault()
        document.getElementById("detailTask").focus()
        document.getElementById("messageErrorTask").innerHTML="* Invalid details, enter details between 3 and 200 characters  *"
    ...
    if (document.getElementById("status").selectedIndex===0){
        ...
        document.getElementById("messageErrorStatus").innerHTML="* Missing status task*"
    ...
    if (!percentageCompleted){
        ...
        document.getElementById("messageErrorPercentageCompleted").innerHTML="* Missing percentage completed *"
    ...
    if (percentageCompleted<=0 || percentageCompleted>100){
        event.preventDefault()
        document.getElementById("percentageCompleted").focus()
        document.getElementById("messageErrorPercentageCompleted").innerHTML="* Invalid forma, the percentage must be between 0 and 100 *"
    ...
    if (document.getElementById("levelOfImportance").selectedIndex===0){
        ...
        document.getElementById("messageErrorlevelOfImportance").innerHTML="* Missing level of Importance *"
    ...
    if (!timeLine){
        ...
        document.getElementById("messageErrorTimeLine").innerHTML="* Missing time Line *"
